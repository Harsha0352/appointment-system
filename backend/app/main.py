...
# Allow all origins (important for ngrok + frontend + friends laptop)
app.add_middleware(
    CORSMiddleware,
    allow_origins=["*"],
...
class AppointmentRequest(BaseModel):
    user_id: str  # Accept string from 11Labs
...
def serialize_row(row):
    if not row:
        return row
    # row is a RealDictRow; convert to plain dict and stringify dates/times
    out = {}
...
# -----------------------------
# Chat endpoint (uses server-side OpenAI key)
# -----------------------------
